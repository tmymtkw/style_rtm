...
class UViT10(BaseBackbone):
    def __init__(self,
        ...
        for _ in range(num_blocks[0]):
            self.before_block.append(NormalEncoderBlock(emb_dims, heads[0], hidden_dims, drop_rate))

        self.down_sample = nn.Sequential(
            nn.MaxPool2d(2, 2),
            nn.Conv2d(emb_dims, emb_dims, kernel_size=1, stride=1),
        ...
        for _ in range(num_blocks[1]):
            self.bottom_block.append(NormalEncoderBlock(emb_dims, heads[1], hidden_dims, drop_rate))

        self.up_sample = UpLayer(emb_dims, emb_dims, self.out_size)

        self.after_block = nn.Sequential()
        for _ in range(num_blocks[2]):
            self.after_block.append(NormalEncoderBlock(emb_dims, heads[0], hidden_dims, drop_rate))

        self.final = nn.Sequential(
            nn.ConvTranspose2d(emb_dims, emb_dims, kernel_size=4, stride=2, padding=1),
            nn.LayerNorm([emb_dims, self.out_size[0]*2, self.out_size[1]*2]),
            nn.GELU()
        )
