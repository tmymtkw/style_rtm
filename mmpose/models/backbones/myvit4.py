...
class MyViT4(BaseBackbone):
    def __init__(self,
        ...
        self.kpt_token = nn.Parameter(torch.zeros(1, num_kpt, emb_dim))
    ...
    def forward(self, x:torch.Tensor):
        ...
        feat_kpt = self.kpt_token.repeat(repeats=(batch_size, 1, 1))

        out = []

        for _ in range(self.num_cycle):
            for layer in self.decoder:
                feat_kpt = layer(feat_kpt, x)
