...
from mmpose.models.utils import IIRConv

@MODELS.register_module()
class CSPNeXtIIR(BaseModule):
    arch_settings = {
    ...
    def __init__(self,
        ...
        self.stem = nn.Sequential(
            IIRConv(3, begin_channels, 3, stride=2),
            nn.BatchNorm2d(begin_channels),
            nn.SiLU(),

            IIRConv(begin_channels, begin_channels, 3),
            nn.BatchNorm2d(begin_channels),
            nn.SiLU(),

            IIRConv(begin_channels, begin_channels*2, 3),
    ...
    def _freeze_stages(self) -> None:
        ...
        for i in [0, 3, 6]:
            self.stem[i].freeze()
        # stage
        for i in range(len(self.arch_settings[self.arch])):
            m = getattr(self, self.layers[i+1])
            m[0][0].freeze()
