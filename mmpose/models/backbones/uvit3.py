...
class UViT3(BaseBackbone):
    def __init__(self,
        ...
        for _ in range(num_blocks[0]):
            self.before_block.append(NormalEncoderBlock(emb_dims[0], head, hidden_dims[0], drop_rate))
        ...
        for _ in range(num_blocks[1]):
            self.bottom_block.append(NormalEncoderBlock(emb_dims[1], head, hidden_dims[1], drop_rate))

        self.up_sample = UpLayer(emb_dims[1], emb_dims[0], self.out_size)

        self.after_block = nn.Sequential()
        for _ in range(num_blocks[2]):
            self.after_block.append(NormalEncoderBlock(emb_dims[0], head, hidden_dims[0], drop_rate))
