...
class RTMCCHead2(BaseHead):
    ...
    def __init__(
        ...
        # self.au_x = nn.Linear(in_channels, W, bias=False)
        # self.au_y = nn.Linear(in_channels, H, bias=False)
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        feats_kpt = self.mlp_kpt(feats_kpt)
        feats_out = feats + feats_kpt
        feats_out = self.act(feats_out)

        pred_x = self.cls_x(feats_out)
        pred_y = self.cls_y(feats_out)
