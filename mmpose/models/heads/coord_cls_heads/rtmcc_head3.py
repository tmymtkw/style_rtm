...
class RTMCCHead3(BaseHead):
    ...
    def __init__(
        ...
        self.loss_distillation = MODELS.build(dict(type='DistillationLoss'))
        ...
        self.mlp = nn.Sequential(
            ScaleNorm(in_channels),
            nn.Linear(in_channels, gau_cfg['hidden_dims'], bias=False))
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        for feat in reversed(feats):
            feat = self.mlp(feat)
            feat = self.gau(feat)
            pred_x = self.cls_x(feat)
            pred_y = self.cls_y(feat)

            if not self.training:
                return pred_x, pred_y
            
            out.append((pred_x, pred_y))
    ...
    def loss(
        ...
        loss_p = 0.
        for pred in out:
            loss_p += self.loss_module(pred, gt_simcc, keypoint_weights)
        loss_p /= len(out)
        losses.update(loss_kpt=loss_p)
        
        loss_d = 0.
        for i in range(len(feats)-1):
            loss_d += self.loss_distillation(feats[i], feats[i+1])
        losses.update(loss_dist=loss_d)
