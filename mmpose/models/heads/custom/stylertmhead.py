...
class StyleRTMHead(BaseHead):
    ...
    def __init__(self,
        ...
        self.transdec = nn.ModuleList()
        for _ in range(num_dec):
            self.transdec.append(DecoderBlock(gau_cfg['hidden_dims'], 
                                              1, 
                                              flatten_dims*gau_cfg['expansion_factor'],
                                              drop_rate=gau_cfg['dropout_rate']))
            
        self.cls_mlp = nn.Linear(cls_num, gau_cfg['hidden_dims'])
